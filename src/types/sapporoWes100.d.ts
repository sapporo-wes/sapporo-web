...
export interface paths {
  ...
  '/runs': {
     ...
     * The `workflow_name` is used to execute the workflow registered in sapporo-service. Please request to `GET /service-ino` to check registered workflows. If `workflow_name` is specified, `workflow_url`, `workflow_type` and `workflow_type_version` will be ignored.
  ...
  '/runs/{run_id}/data/path-to-file-or-dir': {
     ...
     * - In the case of `path/to/dir`, this returns the list of files under directory in JSON format.
     * - In the case of `path/to/dir?download=true`, this returns the download link of the directory in zip format.
     *
     * The path is relative to the base directory of each run. See `README.md` in sapporo-service for the structure of `run_dir`. For example, if you want to download the output `foo.txt`, specify something like `outputs/foo.txt`.
     * `..` will be ignored.
     */
...
export interface definitions {
  ...
  ServiceInfo: {
    ...
    executable_workflows?: definitions['Workflow'][]
  }
  ...
  RunLog: {
    ...
    outputs?: definitions['AttachedFile'][]
  }
  /**
  ...
  RunRequest: {
     ...
     */
    tags?: { [key: string]: string }
    /** OPTIONAL Specify the name of the workflow engine to run workflow. */
    ...
    workflow_url?: string
    /** OPTIONAL The `workflow_name` is used to execute the workflow registered in sapporo-service. Please request to `GET /service-ino` to check registered workflows. If `workflow_name` is specified, `workflow_url`, `workflow_type` and `workflow_type_version` will be ignored. */
    workflow_name?: string
    /** OPTIONAL As a workflow attachment, it returns a list of files attached to the run request. The file_url is generated by using the file distribution feature of the sapporo-service server. */
    workflow_attachment?: definitions['AttachedFile'][]
  }
  RunId: {
    /** workflow run ID */
    run_id?: string
  ...
  /** The information about the workflow provided by `service-info` in the mode of executing only the workflow registered by the administrator. */
  Workflow: {
    /** The workflow name */
    workflow_name?: string
    ...
    /** A map with keys as the workflow format type name (currently only CWL and WDL are used although a service may support others) and value is a workflow_type_version object which simply contains an array of one or more version strings */
    workflow_type_version?: string
    /** The workflow_attachment array may be used to download files that are required to execute the workflow, including the primary workflow, tools imported by the workflow, other files referenced by the workflow, or files which are part of the input. The implementation should stage these files to a temporary directory and execute the workflow from there. */
    workflow_attachment?: definitions['AttachedFile'][]
  }
  AttachedFile: {
  ...
  RunWorkflow: {
    parameters: {
      formData: {
        ...
        workflow_name?: string
        workflow_url?: string
        workflow_attachment?: string[] | string
      }
    }
    responses: {
      200: {
